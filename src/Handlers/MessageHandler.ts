    ...
    handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
        if (!(M.chat === 'dm') && M.WAMessage.key.fromMe && M.WAMessage.status.toString() === '2') {
            ...
            */
            ...
            M.sender.username = this.client.user.name || this.client.user.vname || this.client.user.short || 'bnh Bot'
        ...
        this.client.log(
         ...
         if (!command) return void M.reply('No such command! Do you mean #help?')
